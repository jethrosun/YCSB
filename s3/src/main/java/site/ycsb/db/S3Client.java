...
public class S3Client extends DB {
  ...
  protected Status writeToStorage(String bucket, String key, Map<String, ByteIterator> values,
    ...
    try (InputStream input = new ByteArrayInputStream(destinationArray)) {
      PutObjectRequest putObjectRequest = PutObjectRequest.builder().bucket(bucket).key(key)
          .contentLength((long) totalSize).build();
      ...
      // s3Client.putObject(putObjectRequest, RequestBody.fromInputStream(input,
      // totalSize));
      try {
  ...
  protected Status readFromStorage(String bucket, String key, Map<String, ByteIterator> result,
    ...
    try {
      GetObjectRequest getObjectRequest = GetObjectRequest.builder().bucket(bucket).key(key).sseCustomerKey(ssecLocal)
          .build();
